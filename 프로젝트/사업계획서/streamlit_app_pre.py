...
def render_table(data, field_mapping, title=None, highlight_word=None):
    ...
    for row in rows:
        ...
        for header in headers:
            ...
            table += f"<td style='border: 1px solid #ddd; padding: 8px; text-align:left; font-weight: normal;'>{cell_value}</td>"
        table += "</tr>"
...
def render_overview(data, field_mapping, highlight_word=None):
    ...
    for category, subcategories in overview_data.items():
        if isinstance(subcategories, dict):
            ...
            st.markdown("**{}**:<br>{}".format(translate_field('uniq_desc', field_mapping), uniq_desc_str.replace('\n', '<br>')), unsafe_allow_html=True)
        else:
            if subcategories:
                value = subcategories
                if highlight_word and highlight_word.lower() in value.lower():
    ...
    # overview.market_con 데이터로 그래프 생성
    market_con = {
        year: get_nested_value(data, f"overview.market_con.{year}".split('.'))
        for year in range(2018, 2024)
    }

    years = list(market_con.keys())
    values = [market_con[year] for year in years]

    plt.figure(figsize=(10, 5))
    plt.plot(years, values, marker='o')
    plt.title('Market Conditions Over Years')
    plt.xlabel('Year')
...
def render_canvas_image(doc_id):
    ...
    font_path = r'C:\Users\MSI\Desktop\study\SW_CAMP\프로젝트\사업계획서\NanumGothic.ttf'  # 나눔고딕 폰트 파일의 경로로 수정하세요
    font = ImageFont.truetype(font_path, 35)  # 폰트 크기를 35로 설정
    ...
    if 'search_word' in st.session_state:
        ...
        if filtered_industries:
            ...
            if documents:
                ...
                # 유사한 항목 표시
                similar_documents = [doc for doc in documents if doc['_source']['item_kw'] != selected_item]
                if similar_documents:
                    st.markdown("### 유사한 항목")
                    st.markdown(render_table(similar_documents, field_mapping, highlight_word=search_word), unsafe_allow_html=True)
            else:
